...
const DEFAULT_GATEWAY_PROGRAM_ID =
  "DJoFYDpgbTfxbXBv1QYhYGc9FK4J5FUKpYXAfSkHryXp";
const DEFAULT_COUNTER_PROGRAM_ID =
...
function resolveCounterAuthority(): PublicKey | null {
  if (process.env.COUNTER_AUTHORITY)
    return new PublicKey(process.env.COUNTER_AUTHORITY);
...
async function getGasQuote(
  ...
  const gasLimitUsed =
    toBigInt(gasLimit) === BigInt(0)
      ? toBigInt(await core.BASE_GAS_LIMIT())
      : toBigInt(gasLimit);

  const [gasToken, gasFeeRaw] = await core.withdrawGasFeeWithGasLimit(
    token,
    gasLimitUsed
  );
  ...
  for (const log of receipt.logs) {
    try {
      ...
      if (parsed?.name === "UniversalTxOutbound") {
        ...
        console.log(
          "  txType:       ",
          TX_TYPES[Number(parsed.args.txType)] ?? parsed.args.txType
        );
        console.log(
          "  target:       ",
          Buffer.from(parsed.args.target.slice(2), "hex").toString("hex")
        );
        ...
        console.log(
          "  payload len:  ",
          ((parsed.args.payload.length - 2) / 2).toString(),
          "bytes"
        );
      }
    } catch {
...
export async function withdrawSol(
  ...
  const { gasToken, gasFee, gasLimitUsed } = await getGasQuote(
    ugpc,
    prc20SolToken,
    gasLimit
  );
  ...
  console.log(
    `[Case 1] Withdraw SOL → ${
      typeof solRecipient === "string" ? solRecipient : solRecipient.toBase58()
    }`
  );
  console.log(`  token: ${prc20SolToken}, amount: ${amount}`);
  console.log(
    `  gasToken: ${gasToken}, gasFee: ${gasFee}, gasLimitUsed: ${gasLimitUsed}`
  );
...
export async function executeOnSolana(
  ...
  if (token === ethers.ZeroAddress) {
    throw new Error(
      "UGPC req.token cannot be address(0). Pass the PRC20 token address."
    );
  }
  ...
  if (gasToken.toLowerCase() === token.toLowerCase()) {
    ...
    if (amountBI > BigInt(0))
      await ensureAllowance(signer, token, UGPC_ADDRESS, amountBI);
  }
...
export async function ceaToUeaSol(
  ...
  const gatewayPk =
    typeof gatewayProgramId === "string"
...
export async function ceaToUeaSpl(
  ...
  const gatewayPk =
    typeof gatewayProgramId === "string"
      ? new PublicKey(gatewayProgramId)
      : gatewayProgramId;
  const mintPk = typeof splMint === "string" ? new PublicKey(splMint) : splMint;
  ...
  console.log(
    `[Case 4] CEA → UEA SPL mint:${mintPk.toBase58()}, drain:${drainAmount}`
  );
  console.log(`  topUpAmount: ${topUpAmountBI}`);
  console.log(
    `  gasToken: ${gasToken}, gasFee: ${gasFee}, gasLimitUsed: ${gasLimitUsed}`
...
async function main() {
  ...
  const PRC20_PSOL =
    process.env.PRC20_PSOL ?? process.env.PRC20_SOL_TOKEN ?? DEFAULT_PRC20.pSOL;
  ...
  const GATEWAY_PROGRAM_ID = new PublicKey(
    process.env.GATEWAY_PROGRAM_ID ?? DEFAULT_GATEWAY_PROGRAM_ID
  );
  const COUNTER_PROGRAM = new PublicKey(
    process.env.COUNTER_PROGRAM_ID ?? DEFAULT_COUNTER_PROGRAM_ID
  );
  ...
  console.log(
    `  Counter Authority: ${
      COUNTER_AUTH?.toBase58() ??
      "missing (set COUNTER_AUTHORITY or upgrade-keypair.json)"
    }`
  );
  console.log(
    `  SPL Mint:          ${
      SPL_MINT?.toBase58() ?? "missing (set SPL_MINT or tokens/usdt-token.json)"
    }`
  );
  ...
  const runExecuteSol = async () => {
    if (!COUNTER_AUTH) {
      throw new Error(
        "Counter authority missing. Set COUNTER_AUTHORITY or keep upgrade-keypair.json in repo root."
      );
    }
    ...
    const incrementDiscr = createHash("sha256")
      .update("global:increment")
      .digest()
      .slice(0, 8);
  ...
  const runCeaToUeaSpl = async () => {
    if (!SPL_MINT) {
      throw new Error(
        "SPL mint missing. Set SPL_MINT or keep tokens/usdt-token.json."
      );
    }
