...
contract UniversalGatewayPCTest is Test {
    ...
    function testWithdrawSuccessWithCustomGasLimit() public {
        ...
        vm.prank(user1);
        gateway.withdraw(to, address(prc20Token), amount, gasLimit, fundRecipient);
    ...
    function testWithdrawEventEmissionWithTxType() public {
        ...
        // Expect the UniversalTxWithdraw event with TX_TYPE.FUNDS
        vm.expectEmit(true, true, false, true);
        emit IUniversalGatewayPC.UniversalTxWithdraw(
            user1, // sender
    ...
    function testWithdrawRevertZeroToken() public {
        ...
        vm.prank(user1);
        vm.expectRevert(Errors.ZeroAddress.selector);
        gateway.withdraw(to, address(0), amount, gasLimit, fundRecipient);
    }
    ...
    function testWithdrawRevertInvalidRecipient() public {
        ...
        uint256 gasLimit = DEFAULT_GAS_LIMIT;
        bytes memory to = abi.encodePacked(user2);
        address fundRecipient = address(0); // Zero recipient
    ...
    function testWithdrawAndExecuteSuccessWithCustomGasLimit() public {
        ...
        vm.prank(user1);
        gateway.withdrawAndExecute(target, address(prc20Token), amount, payload, gasLimit, fundRecipient);
    ...
    function testWithdrawAndExecuteEventEmissionWithTxType() public {
        ...
        // Expect the UniversalTxWithdraw event with TX_TYPE.FUNDS_AND_PAYLOAD
        vm.expectEmit(true, true, false, true);
        emit IUniversalGatewayPC.UniversalTxWithdraw(
            user1, // sender
            SOURCE_CHAIN_ID, // chainId
    ...
    function testWithdrawAndExecuteRevertZeroToken() public {
        ...
        vm.prank(user1);
        vm.expectRevert(Errors.ZeroAddress.selector);
        gateway.withdrawAndExecute(target, address(0), amount, payload, gasLimit, fundRecipient);
    }
    ...
    function testWithdrawAndExecuteDifferentPayloadSizes() public {
        ...
        vm.prank(user1);
        gateway.withdrawAndExecute(target, address(prc20Token), amount, smallPayload, gasLimit, fundRecipient);
        ...
        vm.prank(user1);
        gateway.withdrawAndExecute(target, address(prc20Token), amount, largePayload, gasLimit, fundRecipient);
    ...
    function testReentrancyProtection() public {
        ...
        vm.prank(address(reentrantContract));
        reentrantContract.attemptReentrancy(to, amount, gasLimit, fundRecipient);
    ...
    function testReentrancyProtectionWithExecute() public {
        ...
        // Call should succeed (reentrancy protection is for preventing recursive calls during execution)
        vm.prank(address(reentrantContract));
        reentrantContract.attemptReentrancyWithExecute(target, amount, payload, gasLimit, fundRecipient);
    ...
    function testGasFeeCalculationAccuracy() public {
        ...
        _testGasFeeForLimit(amount, to, fundRecipient, 50_000);
        _testGasFeeForLimit(amount, to, fundRecipient, 100_000);
        _testGasFeeForLimit(amount, to, fundRecipient, 200_000);
        _testGasFeeForLimit(amount, to, fundRecipient, 500_000);
        _testGasFeeForLimit(amount, to, fundRecipient, 1_000_000);
    }

    function _testGasFeeForLimit(uint256 amount, bytes memory to, address fundRecipient, uint256 gasLimit)
        internal
    {
        uint256 expectedGasFee = calculateExpectedGasFee(gasLimit);
    ...
    function testInvalidFeeQuoteZeroGasToken() public {
        ...
        gateway.withdraw(to, address(invalidToken), amount, gasLimit, fundRecipient);
    }
    ...
    function testTokenBurnFailure() public {
        ...
        vm.prank(user1);
        vm.expectRevert("MockPRC20: insufficient balance");
        gateway.withdraw(to, address(failingToken), amount, gasLimit, fundRecipient);
    }
