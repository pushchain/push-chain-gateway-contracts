...
contract GatewayTSSFunctionsTest is BaseTest {
    ...
    function testWithdraw_Native_Success() public {
        ...
        bytes32 universalTxID = bytes32(uint256(1024));
        ...
        vm.deal(tss, amount);
        vm.prank(tss);
        gateway.withdraw{ value: amount }(txID, universalTxID, originCaller, user1, amount);
    ...
    function testWithdraw_Native_OnlyTSS() public {
        ...
        bytes32 universalTxID = bytes32(uint256(1025));
        uint256 amount = 1 ether;

        vm.deal(user1, amount);
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(Errors.WithdrawFailed.selector));
        gateway.withdraw{ value: amount }(txID, universalTxID, user1, user2, amount);
    }

    function testWithdraw_Native_ReplayProtection() public {
        bytes32 txID = bytes32(uint256(26));
        bytes32 universalTxID = bytes32(uint256(1026));
    ...
    function testWithdraw_Native_ZeroRecipient_Reverts() public {
        ...
        bytes32 universalTxID = bytes32(uint256(1027));
        uint256 amount = 1 ether;

        vm.deal(tss, amount);
        vm.prank(tss);
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidInput.selector));
        gateway.withdraw{ value: amount }(txID, universalTxID, user1, address(0), amount);
    }

    function testWithdraw_Native_ZeroOriginCaller_Reverts() public {
        bytes32 txID = bytes32(uint256(28));
        bytes32 universalTxID = bytes32(uint256(1028));
        uint256 amount = 1 ether;

        vm.deal(tss, amount);
        vm.prank(tss);
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidInput.selector));
        gateway.withdraw{ value: amount }(txID, universalTxID, address(0), user1, amount);
    }

    function testWithdraw_Native_ZeroAmount_Reverts() public {
        bytes32 txID = bytes32(uint256(29));
        bytes32 universalTxID = bytes32(uint256(1029));

        vm.prank(tss);
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidAmount.selector));
        gateway.withdraw(txID, universalTxID, user1, user2, 0);
    }

    function testWithdraw_Native_AmountMismatch_Reverts() public {
        bytes32 txID = bytes32(uint256(30));
        bytes32 universalTxID = bytes32(uint256(1030));
        uint256 amount = 1 ether;
        uint256 wrongValue = 0.5 ether;
        ...
        vm.prank(tss);
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidAmount.selector));
        gateway.withdraw{ value: wrongValue }(txID, universalTxID, user1, user2, amount);
    }

    function testWithdraw_Native_WhenPaused_Reverts() public {
        bytes32 txID = bytes32(uint256(31));
        bytes32 universalTxID = bytes32(uint256(1031));
        uint256 amount = 1 ether;
    ...
    function testWithdraw_Native_EmitsCorrectEvent() public {
        ...
        bytes32 universalTxID = bytes32(uint256(1032));
        ...
        vm.deal(tss, amount);
        vm.prank(tss);
        gateway.withdraw{ value: amount }(txID, universalTxID, originCaller, recipient, amount);
    }
    ...
    function testWithdraw_Native_MultipleSequentialWithdrawals() public {
        ...
        vm.deal(tss, amount1);
        vm.prank(tss);
        gateway.withdraw{ value: amount1 }(bytes32(uint256(33)), bytes32(uint256(1033)), user2, user1, amount1);

        vm.deal(tss, amount2);
        vm.prank(tss);
        gateway.withdraw{ value: amount2 }(bytes32(uint256(34)), bytes32(uint256(1034)), user2, user1, amount2);
