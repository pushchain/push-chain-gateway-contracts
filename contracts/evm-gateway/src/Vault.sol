    ...
    IVault
{
    ...
    // =========================
    /// @inheritdoc IVault
    function withdraw(bytes calldata txID, address ueaAddress, address token, address to, uint256 amount)
        external
        nonReentrant
        whenNotPaused
        onlyRole(TSS_ROLE)
    ...
    /// @inheritdoc IVault
    function revertWithdraw(bytes calldata txID, address token, address to, uint256 amount, RevertInstructions calldata revertInstruction)
        external
        nonReentrant
        whenNotPaused
        onlyRole(TSS_ROLE)
