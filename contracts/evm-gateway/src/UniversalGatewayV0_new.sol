...
contract UniversalGatewayV0_new is
    Initializable,
    ContextUpgradeable,
    PausableUpgradeable,
    ...
    IUniversalGatewayV0
{
    ...
    address public WETH;
    ISwapRouterSepolia public uniV3Router;                      // Uniswap V3 router.
    ...
    address public USDT;                                       // USDT token address for the old addFunds function
    uint24 public POOL_FEE = 3000;                            // Pool fee for WETH/USDT swap (typically 3000 for 0.3%)
    AggregatorV3Interface public usdtUsdPriceFeed;             // USDT/USD price feed for calculating final USD amount
    ...
    event FundsAdded(address indexed user, bytes32 indexed transactionHash, AmountInUSD AmountInUSD);
    ...
    function _addFunds(bytes32 _transactionHash, uint256 nativeAmount) private {
        ...
        uint256 usdAmount = (uint256(usdtPrice) * usdtReceived) / 10 ** 6;

        AmountInUSD memory usdAmountStruct = AmountInUSD({ amountInUSD: usdAmount, decimals: usdDecimals });
